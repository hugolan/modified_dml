...
cross_attention_module = CrossAttention(query_dim = 2048, context_dim = 512, device = opt.device)
...
to_optim.append({
            'params': cross_attention_module.parameters(),
            'lr': opt.lr,
            'weight_decay': opt.decay
        })
    ...
    for i, out in enumerate(data_iterator):
        ...
        with context:
            ...
            model_args = {
                ...
                'language_embeds': language_guide.language_embeds[sample_indices],
                'device' : opt.device
            }
            ...
            #language_embeds_query_attention = language_guide.language_embeds[sample_indices].to(opt.device)
            #out_cross_attention = cross_attention_module(x = language_embeds_query_attention, context = embeds.to(opt.device)).to(opt.device)
            #embeds = out_cross_attention.squeeze()
            #print(embeds.size())
            #print(out_cross_attention.size())
            #print(embeds)
            #print(out_cross_attention)
...
opt.completed = True
pkl.dump(opt, open(opt.save_path + "/200_eepochs.pkl", "wb"))
