...
class LanguageGuide(torch.nn.Module):
    ...
    def regularize(self, batch, labels, sample_indices=None):
        ...
        if self.iter_count >= self.activation_iter:
            ...
            if False:
                language_embeds = torch.stack([self.language_embeds[idx] for idx in labels.detach().cpu().numpy()])
            ...
            language_embeds = language_embeds.to(batch.device)
...
class ClipLanguageModel(torch.nn.Module):
    ...
    def forward(self, text, device, skip_primer=False):
        ...
        language_embeds = self.custom_encode_text(primed_tokens.to(device), self.model)
        return language_embeds.type(torch.float32)

    def custom_encode_text(self, text, model):
...
def reembed_dict_in_language(language_model, label_dict, device):
    ...
    import json
    with open('drive/MyDrive/model_capdec/dictionaire_final.json') as json_file:
      dictionaire = json.load(json_file)

    label_dict = dictionaire
    
    arr = np.arange(0,len(label_dict))
    ...
    reembed_collect = []
    with torch.no_grad():
        language_embeds = language_model(list(label_dict.keys()), device,
                                         False).cpu()
        unique_labs = {
            key: language_embed
            for key, language_embed in zip(arr, language_embeds)
        }
